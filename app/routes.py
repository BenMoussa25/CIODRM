...
def get_service_results(service_id):
    ...
    # Create incidents from results if they don't already exist
    if results and 'data' in results:
        new_incidents = []
        for result in results['data'].get('results', []):
            # Create an incident from the result
            incident = create_incident_from_result(result, service['name'])
            
            # Check if a similar incident already exists
            existing = Incident.query.filter(
                Incident.timestamp >= incident.timestamp - timedelta(minutes=5),
                Incident.timestamp <= incident.timestamp + timedelta(minutes=5),
                Incident.source_ip == incident.source_ip,
                Incident.type == incident.type,
                Incident.monitoring_system == incident.monitoring_system
            ).first()
        ...
        if new_incidents:
            try:
                db.session.commit()
                formatted_results['new_incidents'] = len(new_incidents)
            except Exception as e:
                db.session.rollback()
                print(f"Error saving incidents: {str(e)}")
    ...
    elif service_id == 'pe_monitor' and results and 'results' in results:
        formatted_results['count'] = len(results['results'])

    return jsonify(formatted_results)

@main_routes.route('/api/monitoring/network_traffic/results')
def get_network_traffic_results():
    from app.utils.monitoring_system import network_monitor
    
    # Get results from the monitor
    results = network_monitor.get_details(
        source_ip=request.args.get('ip'),
        timestamp=request.args.get('timestamp')
    )
    
    # Create incidents from network monitoring results
    if results and 'data' in results:
        incident = Incident(
            timestamp=datetime.utcnow(),
            source_ip=results['data'].get('source_ip', 'Unknown'),
            type='Network Activity',
            severity='Medium',  # Default severity
            ml_model_name='Network Monitor',
            confidence_score=1.0,
            status='New',
            description=f"Network activity detected from {results['data'].get('source_ip', 'Unknown')}",
            monitoring_system='Network Monitor',
        ...
        if not existing:
            try:
                ...
                db.session.rollback()
                print(f"Error saving network incident: {str(e)}")
...
@main_routes.route('/incident/<incident_id>/generate-report', methods=['POST'])
...
def generate_incident_report(incident_id):
    ...
    try:
        report = llm_reporter.generate_incident_report(incident)
        incident.llm_summary = report
        db.session.commit()
...
@main_routes.route('/api/llm-config/test-report', methods=['POST'])
@role_required('admin')
def test_llm_report():
    ...
    llm_reporter = LLMReporter()
    try:
        report = llm_reporter.generate_test_report(prompt)
        return jsonify({
            'status': 'success',
            'report': report
        })
